...
client_ui, _ = loadUiType('client.ui')
...
class Login(QMainWindow, login):
    def __init__(self):
        ...
        self.pushButton_8.clicked.connect(self.signup_page)
        self.pushButton_44.clicked.connect(self.Handel_Signup)
    ...
    def Handel_Login(self):
        ...
        for row in data:
            if username == row[1] and password == row[3]:
                print('user match')
                if row[4]:
                    self.window2 = ClientMainApp()
                    self.close()
                    self.window2.show()
                    self.current_client = row[4]
                else:
    ...
    def check_is_existed(self,table,item):
        sql = "SELECT FROM %s WHERE %s = '%s' " % (table,item)
        self.cur.execute(sql)
        data = self.cur.fetchall()
    ...
    def Handel_Signup(self):
        try:
            ...
            national_id = str(self.lineEdit_13.text())
            phone = str(self.lineEdit_12.text())
            email = str(self.lineEdit.text())
            username = str(self.lineEdit_3.text())
            password = str(self.lineEdit_24.text())
            password_again = str(self.lineEdit_14.text())

            if name and national_id and phone and email and username and password and password_again :

                if password_again != password:
                    self.statusBar().showMessage("Password is not the same at the two fields ")
                    return True
                ...
                data = self.cur.fetchall()
                if data:
                    self.statusBar().showMessage("The Username is used before ")
                    return True
                ...
                sql = '''SELECT id FROM clients WHERE national_id = '%s' ''' % national_id
                self.cur.execute(sql)
                data = self.cur.fetchall()
                if data:
                    sql = " INSERT INTO users (name,email,password,client_id) VALUES (%s,%s,%s,%s)"%(username,email,password,data[0][0])
                    self.cur.execute(sql)
                    self.db.commit()
                    self.db.close()
                    self.statusBar().showMessage('Signup Succeeded')
                    self.window2 = ClientMainApp()
                    self.close()
                    self.window2.show()
                    self.window2.current_client = data[0][0]

        except Exception as e:
            self.statusBar().showMessage(str(e))


class ClientMainApp(QMainWindow, client_ui):
    def __init__(self):
        QMainWindow.__init__(self)
        self.setupUi(self)
        self.Handel_UI_Changes()
    ...
    def Handel_Buttons(self):
        ...
        self.pushButton_25.clicked.connect(self.Open_History_Tab)
        # self.pushButton_9.clicked.connect(self.)
...
''', (db, table))
    ...
    def get_book_from_code(self, code, branch_id):
        query = "SELECT name,id FROM book WHERE code = '%s' AND rented = false and branch_id = %s;" % (code, branch_id)
        self.cur.execute(query)
        data = self.cur.fetchone()
        return data

    def get_book_from_code_retrieve(self, code, branch_id):
        query = "SELECT name,id FROM book WHERE code = '%s' AND rented = true and branch_id = %s;" % (code, branch_id)
        self.cur.execute(query)
        data = self.cur.fetchone()
        return data
...
class MainApp(QMainWindow, ui):
    ...
    def Handel_Buttons(self):
        ...
        #
        self.pushButton_8.clicked.connect(self.Handel_Day_Operations)
    ...
    def Handel_Day_Operations(self):
        ...
        number_of_days = self.lineEdit_7.text()
        type = self.comboBox.currentText()
        branch = self.comboBox_18.currentText()
        today_date = datetime.date.today()
        if type == 'Rent':
            if book_code and client_name and number_of_days and type and branch:
                if self.search_by_name('branch', branch):
                    branch_id = self.search_by_name('branch', branch)
                else:
                    self.statusBar().showMessage('The Branch is not found')
                    return True

                if self.get_book_from_code(book_code, branch_id):
                    data = self.get_book_from_code(book_code, branch_id)
                    print(data)
                    book_id = data[1]
                else:
                    self.statusBar().showMessage('The Book is not found or rented')
                    return True
                if self.search_by_name('clients', client_name):
                    client_id = self.search_by_name('clients', client_name)
                else:
                    self.statusBar().showMessage('The Client is not found')
                    return True
                if not str(number_of_days).isdigit():
                    self.statusBar().showMessage('Rent Duration Must Be Numbers Only')
                    return True
                if int(number_of_days) <= 0:
                    self.statusBar().showMessage('Rent Duration Must Be More Than 0')
                    return True
                date_to = today_date + datetime.timedelta(days=int(number_of_days))
                self.db = mysql.connector.connect(host=db_host, user=db_user, password=db_password, db=db)
                self.cur = self.db.cursor(buffered=True)

                self.cur.execute('''
                    INSERT INTO dayoperations(book_id, client_id,branch_id, type , days , date_from , date_to )
                    VALUES (%s , %s , %s, %s , %s , %s,%s)
                ''', (book_id, client_id, branch_id, type, number_of_days, today_date, date_to))
                sql = " UPDATE book SET rented = true WHERE id = %s  AND branch_id = %s "% (book_id,branch_id)
                self.cur.execute(sql)
                self.db.commit()
            ...
            if book_code and client_name and type and branch:
                if self.search_by_name('branch',branch):
                    branch_id = self.search_by_name('branch',branch)
                else:
                    self.statusBar().showMessage('The Branch is not found')
                ...
                if self.get_book_from_code_retrieve(book_code,branch_id):
                    data = self.get_book_from_code_retrieve(book_code,branch_id)
                    print(data)
                    book_id = data[1]
                else:
                    self.statusBar().showMessage('The Book is not found or not rented to retrieve')
                    return True
                if self.search_by_name('clients',client_name):
                    client_id = self.search_by_name('clients',client_name)
                else:
                ...
                self.cur.execute('''
                    UPDATE dayoperations SET retrieve_date = %s WHERE retrieve_date IS NULL AND client_id = %s AND book_id = %s AND branch_id = %s ORDER BY create_date DESC LIMIT 1
                ''', (today_date,client_id,book_id,branch_id))
                sql = " UPDATE book SET rented = false WHERE id = %s  AND branch_id = %s "% (book_id,branch_id)
                self.cur.execute(sql)
                self.db.commit()
                self.statusBar().showMessage('Book Retrieved')
...
if __name__ == '__main__':
    ...
    executeScriptsFromFile(
        '/home/mohamed/library_management_system/Library-System/db2.sql')
    cnx.commit()
    cursor.close()
    cnx.close()
    ...
    if create_tables:
        executeScriptsFromFile(
            '/home/mohamed/library_management_system/Library-System/db.sql')
        cnx.commit()
        cursor.close()
        cnx.close()
    main()
